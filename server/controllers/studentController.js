...
exports.getQuizDetails = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Error fetching quiz details', error: error.message });
  }
};
...
exports.submitQuiz = async (req, res) => {
  try {
    ...
    if (!quiz || !quiz.is_published) {
      return res
        .status(404)
        .json({ message: 'Quiz not found or not available' });
    }
...
exports.getStudentStats = async (req, res) => {
  try {
    ...
    const quizzesTaken = (
      await QuizSubmission.count({ student_id: studentId })
    )[0].count;
    ...
    const recentActivity = (
      await QuizSubmission.countSince('submitted_at', 7)
    )[0].count;
